...
function BenchAPI_DepProcess_DepInst_JsonQuery() {
    ...
    # use gitee source
    curl -o /tmp/jq.tar.gz https://gitee.com/songdaoyuan/hkzc-dev-ops-tools/raw/master/LinuxBench/jq.tar.gz
...
function BenchAPI_DepProcess_DepInst_FIO() {
    ...
    # use gitee source
    curl -o /tmp/fio.tar.gz https://gitee.com/songdaoyuan/hkzc-dev-ops-tools/raw/master/LinuxBench/fio.tar.gz
    pushd /tmp/ >/dev/null 2>&1 || return 1
...
function BenchAPI_DepProcess_DepInst_Speedtest() {
    ...
    # use gitee source
    curl -o /tmp/speedtest.tar.gz https://gitee.com/songdaoyuan/hkzc-dev-ops-tools/raw/master/LinuxBench/speedtest.tar.gz
    pushd /tmp/ >/dev/null 2>&1 || return 1
...
function BenchAPI_DepProcess_DepInst_NextTrace() {
    [[ "${Flag_DepProcess_DepScan_CurlExist}" != "1" ]] && BenchAPI_DepProcess_DepInst_Curl
    # curl -o /tmp/nexttrace.tar.gz https://raw.githubusercontent.com/LemonBench/LemonBench/main/requirements/nexttrace/nexttrace-1.2.7-"$(arch)".tar.gz
    # use gitee source
    curl -o /tmp/nexttrace.tar.gz https://gitee.com/songdaoyuan/hkzc-dev-ops-tools/raw/master/LinuxBench/nexttrace.tar.gz
    pushd /tmp/ >/dev/null 2>&1 || return 1
...
function BenchAPI_DepProcess_DepInst_WorstTrace() {
    ...
    # use gitee source
    curl -o /tmp/worsttrace.tar.gz https://gitee.com/songdaoyuan/hkzc-dev-ops-tools/raw/master/LinuxBench/worsttrace.tar.gz
    pushd /tmp/ >/dev/null 2>&1 || return 1
